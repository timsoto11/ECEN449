...
//----------------------------------------------------------------------------
  ...
  reg slv_reg1_previous, slv_reg1_current;
  reg interrupt_rst_flag;
  ...
  begin
	...
	begin
		slv_reg1_current  <= slv_reg1;
		slv_reg1_previous <= slv_reg1_current;
		
  		if(slv_reg1_previous < slv_reg1_current) // correct here
		//if(start_flag==1)
		begin	
			Interrupt  <= 1'b1;
			slv_reg2 <= 32'h00008000;
		end
		if(slv_reg2 == 32'h10008000) //reset bit positive, reset Interrupt
		begin
			Interrupt <= 1'b0;
			slv_reg2 <= 32'b0;	
		end
		if ( slv_reg_write_sel == 3'b001 )
		begin
            		for (byte_index=0;byte_index<=(C_SLV_DWIDTH/8)-3;byte_index=byte_index+1 )// write lower 16bits
     	      		  if ( Bus2IP_BE[byte_index] == 1 )// Bus to IP byte enables
      ...
      if ( Bus2IP_Reset == 1 )
        ...
        case ( slv_reg_write_sel )
         3'b100 :
          ...
          3'b001 :
            for ( byte_index = 0; byte_index <= (C_SLV_DWIDTH/8)-3; byte_index = byte_index+1 )
              if ( Bus2IP_BE[byte_index] == 1 )// Bus to IP byte enables
                for ( bit_index = byte_index*8; bit_index <= byte_index*8+7; bit_index = bit_index+1 )
                  slv_reg2[bit_index] <= Bus2IP_Data[bit_index];
          default : ;
