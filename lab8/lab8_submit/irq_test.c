...
int my_init(void)
{
  ...
  printk("physical address is: %d\nvirtual address is: %d\n", PHY_ADDR, virt_addr);
...
static ssize_t device_read(struct file *filp,	/* see include/linux/fs.h   */
			   ...
			   loff_t * offset)
{
    wait_event_interruptible(queue, (curr != NULL)); /*If there's an active interrupt, sleep until it ends*/
  int bytes_read = 0;
...
irqreturn_t irq_handler(int irq, void *dev_id) {
  ...
  if( end == start + 100) end = start;
  // write the values to the peripheral, from kernel space to h/w
  iowrite32(0x10000000, virt_addr + 8); // set slv_reg2[16], the control bit, one
					// clear the interrupt
