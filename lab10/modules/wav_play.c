...
char msg_buff[200];//used to hold read IR messages
/*audio playback mode settings*/
int vol_setting, rate_setting, mono_setting; //hold current audio settings
unsigned int *rate_ptr, *mono_ptr;
...
//flags for settings
static int flag_ch_up = 0;//channel up flag
static int flag_ch_down = 0;//channel down flag
static int flag_vol_up = 0;//volume up flag
static int flag_vol_down = 0;//volume down flag
...
int main()
{
  ...
  {   
    ...
    if(play_audio == ENABLE_PLAY)
    {
      ...
      write(fd_out, buffer, SMAPLE_SIZE);
    }
    ...
    switch(result)
    {
      ...
      case 0x92000000:
        if(flag_vol_up==1)
        {
          ...
          flag_vol_up = 0;// show the volume has been set
        }
      ...
      case 0x93000000:
        if(flag_vol_down==1)
        {
          if(current_volume!=AC97_VOL_ATTN_46_0_DB)// the lowest sound
            current_volume = current_volume + AC97_VOL_ATTN_DELTA_DB;
          flag_vol_down = 0; // show the volume has been set
        }
        else if(wait_count == 0)
          flag_vol_down = 1;
        val = current_volume;
        ...
        if(flag_ch_up==1){
          /*open next wav file in directory*/
			...
			if(nxt_file==num_files)
				nxt_file=0;// go to first file
			flag_ch_up = 0;
        }
        else if(wait_count == 0)
          flag_ch_up = 1;
		printf("channel up complete\n");
        break;
...
static int open_wav_file(char* filename)
{
    ...
    mono_ptr = &mono_setting;
    ioctl(fd_out, ENABLE_DISABLE_MONO, mono_ptr);


    /*set playback rate accordingly*/
    playback_rate = wav_info.sample_rate;
    if(playback_rate==8000)
      rate_setting = AC97_PCM_RATE_8000_HZ;
    else if(playback_rate==11025)
      rate_setting = AC97_PCM_RATE_11025_HZ;
    else if(playback_rate==16000)
      rate_setting = AC97_PCM_RATE_16000_HZ;
    else if(playback_rate==22050)
      rate_setting = AC97_PCM_RATE_22050_HZ;
    else if(playback_rate==44100)
      rate_setting = AC97_PCM_RATE_44100_HZ;
    else if(playback_rate==48000)
      rate_setting = AC97_PCM_RATE_48000_HZ;
	else ;
    //set playback rate
	rate_ptr = &rate_setting;
    ioctl(fd_out, ADJUST_PLAYBACK_RATE, rate_ptr);
